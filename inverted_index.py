...
def merge(temp_count,total_file):
    temp_file = [] 
    temp_line = []
    token_dict = {}
    #load each file
    ...
    while len(temp_line):
        ...
        while f < length:
            if temp_line[f].split()[0] == token:
                ...
                write_line += temp_line[f][len(token) + 1:-1]
                ...
                if line == '':
                    ...
                    temp_line[f] = line
            f += 1
        write_line = write_line.rstrip().split()
        length = int(len(write_line)/2)
        for i in range(length):
            tf = float(write_line[2*i + 1])
            write_line[2*i + 1] = str(tf * math.log(total_file/length+1))
        inverted_index.write(f'{ " ".join(write_line)}\n')
